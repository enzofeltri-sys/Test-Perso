...
class PortfolioPaperTrader:
    def __init__(self, exchange_id: str, symbols: list, timeframe: str, strategy_factory,
        ...
        self.positions = {s: None for s in symbols}  # None ou {qty, entry_price, stop_price, target_price}
    ...
    def _step(self):
        ...
        equity = self.cash + sum(
            self.positions[s]["qty"] * prices[s] for s in rows if self.positions.get(s) is not None
        )
        ...
        for s in list(rows.keys()):
            ...
            elif self.strategies[s].should_exit_on_signal(row):
            ...
            if exit_price is not None:
                ...
                self.strategies[s].on_position_closed()
        ...
        open_count = sum(1 for s in self.symbols if self.positions.get(s) is not None)

        if breaker_ok and dd_ok:
            candidates = []
            for s in rows:
                if self.positions.get(s) is not None:
                    continue
            ...
            for s, _ in candidates:
                ...
                held_symbols = [s2 for s2 in self.symbols if self.positions.get(s2) is not None]
        ...
        print(f"[{datetime.now()}] équity totale = {total_equity:.2f}  "
              f"positions ouvertes = {open_count}/{len(self.symbols)}")
